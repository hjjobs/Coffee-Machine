...
public class Coffee implements Beverage {
    ...
    public void dispenseDrink(Map<Ingredients, Integer> currentItemsQuantity, Map<Ingredients, Integer> reqiuredItemsQuantity) {
        if (isMakeable(Beverages.HOT_COFFEE, currentItemsQuantity, reqiuredItemsQuantity)) {
            System.out.println(Beverages.HOT_COFFEE.getValue() + IS_PREPARED);
            try {
                Files.write(Paths.get("../testproject/src/main/resources/output.txt"), (Beverages.HOT_COFFEE.getValue() + IS_PREPARED + "\n").getBytes(), StandardOpenOption.APPEND);
            } catch (IOException e) {
