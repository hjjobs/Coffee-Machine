...
public class GreenTea implements Beverage {
    ...
    public void dispenseDrink(Map<Ingredients, Integer> currentItemsQuantity, Map<Ingredients, Integer> reqiuredItemsQuantity) {
        if (isMakeable(Beverages.GREEN_TEA, currentItemsQuantity, reqiuredItemsQuantity)) {
            System.out.println(Beverages.GREEN_TEA.getValue() + IS_PREPARED);
            try {
                Files.write(Paths.get("../testproject/src/main/resources/output.txt"), (Beverages.GREEN_TEA.getValue() + IS_PREPARED + "\n").getBytes(), StandardOpenOption.APPEND);
            } catch (IOException e) {
